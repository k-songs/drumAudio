...
import { DRUM_SOUNDS, DIFFICULTY_LEVELS } from '../constants/drumSounds';
...
function TempDrumGame({ difficulty = 'beginner', onGameComplete }) {
  ...
  const startNewRound = () => {
    ...
    // 전체 선택지 섞기
    const allChoices = [correctInstrument, ...wrongChoices]
      .sort(() => 0.5 - Math.random());
  ...
  const playSound = async () => {
    try {
      ...
      const drumData = DRUM_SOUNDS[currentInstrument];

      // 실제 MP3 파일이 있으면 사용, 없으면 테스트 사운드 사용
      const soundSource = drumData.sound || { uri: drumData.testSound };
      ...
      if (!drumData.sound) {
        setTimeout(() => {
          Alert.alert(
            '테스트 모드',
            `${drumData.name} (${drumData.frequency}Hz)\\n실제로는 MP3 파일이 재생됩니다.`,
            [{ text: '확인' }]
          );
        }, 100);
      }
  ...
  const handleAnswer = (selectedInstrument) => {
    ...
    const correctDrum = DRUM_SOUNDS[currentInstrument];
    const selectedDrum = DRUM_SOUNDS[selectedInstrument];
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.difficulty}>{currentDifficulty.name} - {currentDifficulty.description}</Text>
      </View>
        ...
        {currentInstrument && (
          <View style={styles.instrumentDisplay}>
            <Text style={styles.instrumentEmoji}>
              {DRUM_SOUNDS[currentInstrument].emoji}
            </Text>
            {gameState === 'ready' && (
            ...
            {choices.map((instrument, index) => (
              ...
              >
                <Text style={styles.choiceButtonText}>
                  {DRUM_SOUNDS[instrument].emoji} {DRUM_SOUNDS[instrument].name}
                </Text>
                <Text style={styles.choiceDescription}>
                  {DRUM_SOUNDS[instrument].description}
                </Text>
              </TouchableOpacity>
            ))}
          </View>
