...
export default function App() {
        ...
        {Object.entries(DIFFICULTY_LEVELS).map(([key, level]) => (
          <TouchableOpacity
            ...
            style={[
              ...
              key === 'beginner' ? styles.beginnerButton : styles.intermediateButton
            ]}
  ...
  const renderResults = () => {
    ...
    const getGrade = () => {
      if (percentage >= 90) return { grade: '최우수', emoji: '🏆', color: '#FFD700' };
      if (percentage >= 80) return { grade: '우수', emoji: '🥇', color: '#C0C0C0' };
      if (percentage >= 70) return { grade: '양호', emoji: '🥈', color: '#CD7F32' };
      if (percentage >= 60) return { grade: '보통', emoji: '🥉', color: '#4CAF50' };
      ...
      <View style={styles.resultsContainer}>
        <View style={styles.resultsCard}>
          ...
          <Text style={styles.encouragementText}>
            {percentage >= 80 ? '훌륭한 청력을 가지고 계시네요!' :
             percentage >= 60 ? '좋은 결과입니다. 더 연습해보세요!' :
             '연습을 통해 청력을 향상시켜보세요!'}
          </Text>
        </View>
